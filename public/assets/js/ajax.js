...
// search interview using ajax
function interview_search(page=null, per_page=null){
	e = window.event;
	e.preventDefault();
	var adm_id = document.getElementById("admission-id").value;
	var panel_id = document.getElementById("panel-id").value;
	var state = document.getElementById("state").value;
	var xhr = new XMLHttpRequest();
	var tbody =document.getElementById('tbody');
	xhr.open("POST",base_url+"api/interview/search",true);
	xhr.setRequestHeader("Content-Type", "application/json");
	var func = "interview_search";
	var route = "interview/list";
	...
	if(page == null){
		var data = {adm_id:adm_id,panel_id:panel_id,state:state};
	}else{
		var data = {adm_id:adm_id,panel_id:panel_id,state:state,page:page,per_page:per_page};
	}
	xhr.send( JSON.stringify(data) );
}
...
// for marks page
function marks_classroom_search(page=null, per_page=null){
	e = window.event;
	e.preventDefault();
	var id = document.getElementById("classroom-id").value;
	var grade = document.getElementById("grade").value;
	...
	var xhr = new XMLHttpRequest();
	var tbody =document.getElementById('tbody');
	xhr.open("POST",base_url+"api/marks/classroom/search",true);
	xhr.setRequestHeader("Content-Type", "application/json");
	var func = "marks_classroom_search";
	var route = "classroom/list";
...
function classroom_marks_search(){
	window.event.preventDefault();
	var form = new FormData(document.getElementById("marks_filter"));

	fetch(base_url+"api/marks/classroom/student/search", {
		method : 'post',
		body : form
	}).then( (res) => {
		return res.text();
...
// get class list for specific section
function get_class_list(grade_input, target){
	classroom_search();
