  ...
  <div class="col-4 h-60 d-block justify-content-center align-items-center bg-white hmpg-b-right">
    ...
    <div class="col-4 h-60 justify-content-start align-items-start bg-white">
      ...
      <div class="notice-board col-12 h-40 border b-rad bg-navyblue1 ">
        ...
        <!--<div class="notices d-block col-12 h-30 border b-rad bg-white">
          
          <div class="row d-flex align-items-start justify-content-start text-justify" >
          <p><?php if(!empty($header)){echo $header['notice_1'];} ?></p>
          </div>
        ...
        <div class="notices col-12 h-30 border b-rad bg-white">
          
          <div class="row d-flex align-items-start justify-content-start text-justify" >
          <p><?php if(!empty($header)){echo $header['notice_2'];} ?></p>
          </div>
        </div>
        ...
        <div class="notices col-12 h-30 border b-rad bg-white">
          ...
          <div class="row d-flex align-items-start justify-content-start text-justify" >
          <p><?php if(!empty($header)){echo $header['notice_3'];} ?></p>
          </div>
        </div>-->
          <?php
          /*$st1 = "SELECT `name`,`value` FROM `notice` WHERE `category`='notice'";
          $notice_result = $con->pure_query($st1)->fetchAll();
            if($notice_result){
               foreach ($notice_result as $dt) {
                  $notice[$dt['name']] = $dt['value'];
              }
            }

                  for ($x = 1; $x <= 5; $x++) {
                    if(isset($notice['']) && isset($notice[''])){
                      echo '        <div class="notices col-12 h-30 fs-12 border b-rad bg-white">
                                      <div class="row d-flex align-items-start justify-content-start text-justify" >
                                      <p>';if(!empty($notice)){echo $notice['notice_1_image'];} echo '</p>
                                      </div>
                                    </div>';
                      }
    ...
    foreach ($rls_set as $data1) {
      ...
      $notice[$data1['id']."_reference"] = $data1['reference'];
                  ...
                  for ($x = 1; $x <= $notice["1000_text"]; $x++) {
                      echo '        <div class="row notices h-30 ">
                                        <div class="row">
          ...
          <div class="d-flex col-12 justify-content-between fs-white">
            ...
            <div class="p-2 ">
             <div class="row justify-content-center">
              ...
              <!--<span class="dot m-2 " onclick="currentNotice(1)"></span>
              <span class="dot m-2" onclick="currentNotice(2)"></span>
              <span class="dot m-2" onclick="currentNotice(3)"></span>-->
                  <?php
                  for ($x = 1; $x <= $notice["1000_text"]; $x++) {
