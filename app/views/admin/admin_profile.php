...
<?php
	...
	if(isset($_POST['submit'])){
		...
		$required_fields = array("username"=>20,"address"=>100,"email"=>100,"contact-number"=>10);
		...
		if(empty($all_errors[0]) && empty($all_errors[1]) && isset($_FILES['profile-photo']['tmp_name'])&& !empty($_FILES['profile-photo']['tmp_name'])){
			$target = "../img/admin_profile_photo/";
		...
		if((empty($all_errors[0]) && empty($all_errors[1])) && (!isset($errors) || $errors[0] == 1)){
			try{
				...
				$con->get(array("email"));
				$result = $con->select("admin", array("id"=>$_SESSION['user_id']));
				if($result && $result->rowCount() == 1){
					$old_email = stripslashes($result->fetch()['email']);
					$result = $con->update("admin",$data,array("id"=>$_SESSION['user_id']));
					if(!$result){
					...
					if($old_email != $data['email']){
						...
						if(!$result){
							throw new PDOException("Login update failed.");
						}
					}
...
<div id="content" class="col-11 col-md-8 col-lg-9 flex-col align-items-center justify-content-start">
	...
	<div class="col-12">
			<form action="admin_profile.php" method="post" class="col-12" enctype="multipart/form-data">
				<div class="col-4 flex-col  d-none d-md-flex align-items-center"  style=" padding-top: 100px;background: #ccf;">
					<div  class="col-12">
						<div  style="position: relative;">
							<img src="<?php echo set_url('img/admin_profile_photo/'.$_SESSION['profile_photo']); ?>" alt="profile photo" onclick="upload_profile_photo('profile-photo')"  class="w-100">
